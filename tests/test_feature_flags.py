...
from feature_flags import FEATURE_FLAGS, FeatureFlags, rbac_unleash_last_poll_timestamp, rbac_unleash_poll_total
...
class FeatureFlagsTest(TestCase):
    ...
    @override_settings(UNLEASH_REFRESH_INTERVAL=45, UNLEASH_REQUEST_TIMEOUT=20)
    ...
    def test_init_uses_configurable_refresh_interval(self, mock_unleash_cls):
        ...
        FEATURE_FLAGS.client = None
        FEATURE_FLAGS._init_unleash_client()
    ...
    @override_settings(UNLEASH_REFRESH_INTERVAL=30, UNLEASH_REQUEST_TIMEOUT=30)
    @patch("feature_flags.UnleashClient")
    def test_init_default_refresh_interval(self, mock_unleash_cls):
        """Test that _init_unleash_client uses default 30s refresh_interval."""
        mock_client = MagicMock()
        mock_unleash_cls.return_value = mock_client
    ...
    def test_on_unleash_event_tracks_fetched(self):
        ...
        before_count = rbac_unleash_poll_total.labels(status="success")._value.get()
        ...
        after_count = rbac_unleash_poll_total.labels(status="success")._value.get()
    ...
    def test_on_unleash_event_updates_timestamp(self):
        ...
        ts = rbac_unleash_last_poll_timestamp._value.get()
